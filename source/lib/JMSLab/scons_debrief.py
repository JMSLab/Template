...
def state_of_repo(maxit, outfile='state_of_repo.log'):
    with open(outfile, 'wb') as f:
        f.write(dedent(f"""
            WARNING: Information about .sconsign.dblite may be misleading
...
def issue_size_warnings(look_in = ['source', 'raw', 'release'],
    ...
    if lfs_required:
        ...
        # if there are large versioned files not tracked by git-lfs,
        # warned the user and perform tracking if the user agrees
        if new_add_list:
            msg = _red_and_bold("Warning:") + \
